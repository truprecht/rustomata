...
where
    A: Clone + Hash + Ord + PartialEq,
    T: Clone + Eq + Hash + Ord,
    W: AddAssign + Clone + Eq + Ord + Zero,
{
    pub fn new<It>(transitions: It, initial: PushDown<A>) -> Self
    where
        It: IntoIterator<Item = Transition<PushDownInstruction<A>, T, W>>,
    {
...
mod tests {
    ...
    fn test_pushdown_instruction_map_correctness() {
        let instruction = PushDownInstruction::Replace {
            current_val: vec![1, 2, 3],
            new_val: vec![1, 2, 3, 4],
        };
        let control_instruction = PushDownInstruction::Replace {
            current_val: vec![2, 4, 6],
            new_val: vec![2, 4, 6, 8],
        };

        assert_eq!(control_instruction, instruction.map(&|x| x * 2));
    }
    ...
    fn test_pushdown_instruction_map_inverse() {
        ...
        assert_eq!(instruction, mapped_instruction.map(&|x| x / 2));
    }
    ...
    fn test_map_vec_mut_correctness() {
        ...
        assert_eq!(
            vec![2, 4, 6, 8],
            map_vec_mut(&vector, &mut |x| {
                modified = true;
                x * 2
            })
        );
        assert_eq!(true, modified);
    }
    ...
    fn test_map_mut_vec_inverse() {
        let vector = vec![1, 2, 3, 4];
        let mut modified = false;
        let mapped_vector = map_vec_mut(&vector, &mut |x| {
            modified = true;
            x * 2
        });
        ...
        assert_eq!(
            vector,
            map_vec_mut(&mapped_vector, &mut |x| {
                modified = false;
                x / 2
            })
        );
        assert_eq!(false, modified);
    }
    ...
    fn test_pushdown_instruction_map_mut_inverse() {
        ...
        let mut modified = false;
        let mapped_instruction = instruction.map_mut(&mut |x| {
            modified = true;
            x * 2
        });
        ...
        assert_eq!(
            instruction,
            mapped_instruction.map_mut(&mut |x| {
                modified = false;
                x / 2
            })
        );
    ...
    fn test_pushdown_instruction_inverse() {
        ...
        assert_eq!(vec![pushdown], inverse_instruction.apply(modified_pushdown));
    }

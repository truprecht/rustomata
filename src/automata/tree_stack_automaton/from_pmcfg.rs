...
impl<X: Display> Display for PosState<X> {
    fn fmt(&self, f: &mut Formatter) -> fmt::Result {
        match *self {
            PosState::Designated => write!(f, "@"),
            PosState::Initial => write!(f, "I"),
            PosState::Position(ref x, i, j) => write!(f, "({}, {}, {})", x, i, j),
        }
    }
}
    ...
    > From<PMCFG<N, T, W>> for TreeStackAutomaton<PosState<PMCFGRule<N, T, W>>, T, W>
{
    fn from(g: PMCFG<N, T, W>) -> Self {
        ...
        let mut rule_map: BTreeMap<N, Vec<PMCFGRule<N, T, W>>> =
            BTreeSet::from_iter(g.rules.iter().map(|r| r.head.clone()))
                .iter()
                .map(|n| (n.clone(), Vec::<PMCFGRule<N, T, W>>::new()))
                .collect();

        let mut down_info: RuleCallerMap<N, T, W> = BTreeMap::new();

        let mut initial_rules: Vec<PMCFGRule<N, T, W>> = Vec::new();
        ...
        let mut agenda: Vec<DerivationSnippet<N, T, W>> = Vec::new();
        ...
        TreeStackAutomaton::new(transitions, TreeStack::new(PosState::Initial))
    }
}

pub fn to_abstract_syntax_tree<A>(
    (tree_map, _): (GornTree<PosState<A>>, Vec<usize>),
) -> GornTree<A> {
    ...
    for (address, pos_state) in tree_map {
        ...
        if let PosState::Position(value, _, _) = pos_state {
            ...
            panic!(
                "The given tree map contains 'designated' or 'initial' nodes that are not the root!"
            );
        }
    }
